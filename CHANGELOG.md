...
- New e2e coverage: `e2e/checkbox-mode.spec.ts` + `/test/checkbox-mode.html`, `e2e/state-persistence.spec.ts`, a no-drop-node section in `/test/drag-drop.html`, and leaf↔folder toggle-transition tests in `e2e/branch-operations.spec.ts`.
